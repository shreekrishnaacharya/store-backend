...
import { returnPurchase, getPurchaseDetail } from "../service";
...
const PurchaseReturn = () => {
    ...
    async function onSubmitHandler(fdata) {
        await returnPurchase(id, fdata).then(({ flag, data }) => {
            if (flag) {
                enqueueSnackbar("Purchase return success", {
                    variant: 'success',
                });
                history.push({
                ...
                enqueueSnackbar("Purchase return failed", {
                    variant: 'error',
                });
            }
        })
    ...
    useEffect(() => {
        if (Object.keys(purData).length === 0 && id !== undefined) {
            loadData();
        }
        return () => setPurData({});
    }, []);
    ...
    useEffect(() => {
        if (purData.pitems) {
            purData.pitems.forEach((val, i) => {
                append({ itemid: val.fitm, name: val.name, cost: val.rate, qty: val.qty });
            });
        }
    }, [purData]);
    ...
    function updateCalculate() {
        ...
        const disAmt = parseFloat(purData.dis_amt);
        const vatRate = parseFloat(purData.vat_rate);
                                        ...
                                        >
                                            ...
                                            <Box component="tfoot">
                                                ...
                                                <TableRow style={{ border: "none" }}>
                                                    ...
                                                    <TableCell style={{ textAlign: "right" }}>
                                                        <SuiTypography variant="subtitle2" component="div">-{purData.dis_amt}</SuiTypography>
                                                    </TableCell>
                                                    <TableCell></TableCell>
                                                </TableRow>
                                                <TableRow style={{ border: "none" }}>
